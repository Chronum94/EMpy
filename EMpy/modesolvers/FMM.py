...
class FMMMode2d(Mode):
    ...
    def fields(self, x=None, y=None):
        ExTE, ExTM, EyTE, EyTM, EzTE, EzTM, cBxTE, cBxTM, cByTE, cByTM, cBzTE, cBzTM = self.eval(
            x, y
        )
        Ex = ExTE + ExTM
        Ey = EyTE + EyTM
...
def FMM1d_y(Uy, ny, wl, nmodi, boundaryRL, TETM, verbosity=0):
    ...
    rekz2 = numpy.linspace(
        (numpy.max(numpy.real(betay)) + searchinterval) ** 2, 0., Nstepskz
    )
    ...
    if len(zerire) < nmodi:
        ...
        while len(kz1) < nmodi:
            ...
            kz1 = numpy.r_[
                kz1,
                1j * (zeriim * k0 - imsearchinterval / 2. + 1j * searchinterval / 2.),
            ]
            kz2 = numpy.r_[
                kz2,
                1j * (zeriim * k0 + imsearchinterval / 2. - 1j * searchinterval / 2.),
...
def FMM1d_x_component(slices, nmodi, verbosity=0):
    ...
    rekz = numpy.linspace(
        ...
        0.,
        10. * numpy.floor((maxrekz + searchinterval) / searchinterval),
    )
